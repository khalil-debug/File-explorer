...
public class Tags {
    ...
    //fonction pour l'affichage de la liste des tags pour l'administrateur
    ...
    //fonction pour l'affichage de la recherche des utilisateurs par tag pour l'administrateur
    public ObservableList<Tags> afficherRechercheTag(String u) throws SQLException{
        ...
        try{
            con= DriverManager.getConnection("jdbc:mysql://localhost:3306/gestionfichiers", "root","");
            s=con.createStatement();
            rs= s.executeQuery("SELECT Id from tags where tag like '"+u+"%' group by id");
            while(rs.next())
            {   
